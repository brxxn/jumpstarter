...
export default class NgrokStopAllCommand extends DiscordCommandHandler {

  constructor() {
    super('ngrokstopall', {
      name: 'ngrokstopall',
      description: 'Stop all active ngrok services'
    });
  }
  ...
  async execute(interaction: ChatInputCommandInteraction, config: JumpstarterConfiguration, client: Client) {
    ...
    await interaction.reply({
      content: ':stop_sign: Successfully stopped all active ngrok services.'
    });
  }
